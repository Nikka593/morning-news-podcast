...
- 難しい言葉は一言かみ砕く
- 全体で必ず{len(items)*300:,}〜{len(items)*380:,}文字にする（1本あたり300〜380文字）。
  確認されている事実の範囲での背景情報（経緯・数字・関係者）を補足して、各ニュースをしっかり膨らませる
- 「ビジネスモデル・事例」の話題では、収益の仕組み（誰から・何で・どう稼ぐか）や
  収益性が改善した要因を、報じられている事実の範囲で説明する
...
def main():
    ...
    if os.environ.get("GEMINI_API_KEY"):
        try:
            ...
            if len(script) < len(items) * 260:  # 目標尺に足りない場合は一度だけ膨らませ直す
                print(f"台本が{len(script)}文字と短いため、増量を再依頼")
                script = generate_with_gemini(
                    prompt
                    + f"\n\n【重要】先ほどの案は{len(script)}文字で短すぎました。"
                    + f"各ニュースに背景や補足を加え、全体を必ず{len(items)*300}文字以上にしてください。"
                )
            print("台本生成: Gemini")
